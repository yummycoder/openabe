...
OpenABEContextOPDH::multiEncryptKEM(OpenABERNG *rng, const vector<string>& pkIDs,
  ...
  OpenABE_ERROR result = OpenABE_NOERROR;
  OpenABERNG *myRNG = this->getRNG();
  OpenABEByteString DerivedKeyMaterial, ctHdr, ct, tag, Z;
  try {
    // check inputs
    ASSERT_NOTNULL(senderID);
    ASSERT_NOTNULL(ciphertext);
...
OpenABE_ERROR
OpenABEContextOPDH::multiEncryptKEMNoList(OpenABERNG *rng, const vector<string>& pkIDs,
                           OpenABEByteString *senderID, uint32_t keyBitLen,
                           const std::shared_ptr<OpenABESymKey> &key,
                           OpenABECiphertext *ciphertext, unique_ptr<oabe::crypto::OpenABEMultiEveSymKeyAuthEnc> &symmEnc) {
  OpenABE_ERROR result = OpenABE_NOERROR;
  ...
  try {
    ...
    for(vector<string>::const_iterator it = pkIDs.begin();
      ...
      // get receiver name
      string receiver = getUser(*it);
      ciphertext->setComponent("Key" + to_string(i), &ct);
      ciphertext->setComponent("Tag" + to_string(i), &tag);
      i++;
...
OpenABE_ERROR
OpenABEContextOPDH::multiDecryptKEMNoList(const string &pkID, const string &skID,
                           OpenABECiphertext *ciphertext, uint32_t keyBitLen,
                           const std::shared_ptr<OpenABESymKey> &key,
                           unique_ptr<oabe::crypto::OpenABEMultiEveSymKeyAuthEnc> &symmEnc) {
  OpenABE_ERROR result = OpenABE_NOERROR;
  ...
  try {
    ...
    vector<string> cipherCompKey = ciphertext->getKeys();
    bool rightReceive = false;
    string pos;
    for (vector<string>::iterator iter = cipherCompKey.begin(); iter != cipherCompKey.end(); iter++) {
      if ((*iter).rfind("Key", 0) == 0) {
          // component key starts with "KEY"
          pos = (*iter).substr(3);
          ek = ciphertext->getByteString(*iter);
          tag = ciphertext->getByteString("Tag" + pos);
          if (!symmEnc->decrypt(planSymmk, &DerivedKeyMaterial, ek, tag)) {
            continue;
          } else {
            rightReceive = true;
    ...
    //not right receiver
    if (!rightReceive) {
      result = OpenABE_NOT_RIGHT_RECEIVER;
      return result;
    }
...
OpenABE_ERROR
OpenABEContextSchemePKE::multiEncryptNoList(OpenABERNG *rng, const vector<string>& pkIDs,
                             const string &senderpkID, const string &plaintext,
                             OpenABECiphertext *ciphertext) {
  OpenABE_ERROR result = OpenABE_NOERROR;
  shared_ptr<OpenABEKey> senderPK = nullptr;
  ...
  try {
    ...
    senderID = senderPK->getUID();
    // Returns a ciphertext and a symmetric key
    result = this->m_KEM_->multiEncryptKEMNoList(rng, pkIDs, &senderID,
                                      DEFAULT_SYM_KEY_BITS, key, ciphertext, symmEnc);
    // Propagate errors from encryptKEM
    ASSERT(result == OpenABE_NOERROR, result);
...
OpenABE_ERROR
OpenABEContextSchemePKE::multiDecryptNoList(const string &pkID, const string &skID,
                             string &plaintext, OpenABECiphertext *ciphertext) {
  OpenABE_ERROR result = OpenABE_NOERROR;
  shared_ptr<OpenABESymKey> key(new OpenABESymKey);
  OpenABEByteString *iv = nullptr, *ct = nullptr, *tag = nullptr;
  ...
  try {
    ...
    result = this->m_KEM_->multiDecryptKEMNoList(pkID, skID, ciphertext,
                                      DEFAULT_SYM_KEY_BITS, key, symmEnc);
    // propagate errors from decryptKEM
    ASSERT(result == OpenABE_NOERROR || result == OpenABE_NOT_RIGHT_RECEIVER, result);
