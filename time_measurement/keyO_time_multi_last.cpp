...
int main(int argc, char* argv[])
{
    ...
    std::ofstream fout("../result/keyO_time_multi(last)(attrLen:" + to_string(attr_length) + "msgLen:" + to_string(message_length) + ").txt", std::ios_base::app);
    ...
    pkScheme.multiEncrypt(attrs[0], attrs, 1, message, ct);
    
    int count = 0;

    string wrong_attr = genRandomString(attr_length);

    //warmup
    pkScheme.multiDecrypt(attrs[0], wrong_attr, ct, dec);
    pkScheme.multiDecrypt(attrs[0], attrs[0], ct, dec);
